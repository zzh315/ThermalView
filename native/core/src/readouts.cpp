...
Readouts computeReadouts(const uint16_t* image, const TemperatureLut& lut, const Region& r,
                         uint16_t clipRaw, const std::vector<uint8_t>* bad, RawWindow window) {
  ...
  for (int y = r.y0; y < r.y1; ++y) {
    ...
    for (int x = r.x0; x < r.x1; ++x) {
      ...
      if (!window.contains(v)) continue;
      if (v < lo) { lo = v; loX = x; loY = y; }
      if (v > hi) { hi = v; hiX = x; hiY = y; }
    }
  }
  Readouts out;
  if (loX >= 0) {  // (none: an empty region, every pixel bad, or none inside the window)
    out.low = spotAt(image, lut, loX, loY, clipRaw);
    out.high = spotAt(image, lut, hiX, hiY, clipRaw);
  } else {
    out.low.x = out.low.y = out.high.x = out.high.y = NAN;
  }
...
Spot ReadoutFilter::follow(const Spot& extreme, Spot& marked, bool higherWins, const uint16_t* image,
                           const TemperatureLut& lut, const Region& region, uint16_t clipRaw,
                           RawWindow window) const {
  // The marked pixel's current reading; move to the new extreme only if it wins by the margin (or
  // the marked pixel has left the region or the window).
  const bool markedUsable = std::isfinite(marked.x) && std::isfinite(marked.y) &&
                            region.contains(int(marked.x), int(marked.y)) &&
                            window.contains(image[size_t(marked.y) * kFrameWidth + size_t(marked.x)]);
...
Readouts ReadoutFilter::update(const Readouts& frame, const uint16_t* image, const TemperatureLut& lut,
  ...
  const Spot high = follow(frame.high, markedHigh_, true, image, lut, region, clipRaw, window);
  const Spot low = follow(frame.low, markedLow_, false, image, lut, region, clipRaw, window);
