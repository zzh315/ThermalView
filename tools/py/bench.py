...
- bench/results/<label>/<scene>.jpg: half-size contact sheets (committed), left out for scenes the
  pipeline leaves (nearly) identical to the baseline.
...
def contact_sheet(scene, outputs, results_dir, upscale):
    ...
    sheet.save(full)
    # Commit a half-size copy unless the pipeline left this scene (nearly) as the baseline has it:
    # under 0.05 levels of mean difference, e.g. a few replaced bad pixels.
    same = "pipeline" in outputs and float(np.abs(outputs["pipeline"] - outputs["baseline"]).mean()) * 255 < 0.05
    if not same:
