...
- Normal-range correction coefficients: m = 1, b = 0. ht301_hacklib's high range uses m = 1.17, b = −40.9, an empirical fit. We now use the high range (owner decision), and M2 picks between that fit and InfiCam's variant by measurement ("Ranges"). Both are implemented in `native/core/temperature`, with the ht301 variant golden-tested against the oracle.
