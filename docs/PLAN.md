...
4. **Temporal denoise.** *Removed (owner, 2026-09-25): it left after-images when the camera moved and gave no visible gain; PIPELINE_LOG.* A per-pixel recursive filter with motion adaptation: the blend weight comes from |current − filtered| relative to the noise level measured on `flat`. Reset on shutter cycles and on large global motion. Goal: big `temporal_noise` gains on static scenes with no ghost trails in `motion`. The camera already filters on board (DEVICE.md "Onboard filtering"): its static noise is correlated from frame to frame (ρ ≈ 0.72, like a recursive filter with weight ~0.28 on the new frame), yet big steps pass within about a frame. So the noise we get is already smoothed in time, and a second filter gains less than it would on white noise; measure what it still buys, and how the camera's filter treats small moving differences on `night`.
   **4b. Spatial noise reduction** (owner request, 2026-09-25, after stage 4's removal: within each frame, so nothing can ghost; PIPELINE_LOG).
   - **Method:** non-local means, chosen in a study against guided filters, Lee, bilateral and wavelet shrinkage. 5×5 patches, an 11×11 search, strength h = 1.1 × the camera's noise σ, measured from consecutive raw frames.
   - **Where it runs:** on the GPU, a compute shader (`native/android/gpu_nlm*`). native/core's CPU version is its reference and its fallback, at a 5×5 search with h scaled to match.
   - **Status:** approved by the owner and on by default; the strength stays a debug setting.
   - **What it leaves:** the per-frame column and row noise, which stands out more once the grain is gone. Parked (owner, 2026-09-25) for a frame-to-frame estimate.
5. **Tone mapping.**
...
- Palettes — control points in `palettes/*.json`, baked into the 1024-entry LUT:
  ...
  - `rainbow_deep` and `rainbow_soft` (owner, 2026-09-26): rainbow_hc's hues at each stop's own lightness, with 90% and 75% of the chroma sRGB allows there. They are the app's Rainbow, Deep by default.
  - `rainbow_hti` (owner, 2026-09-26: "HTi's rainbow_hc has blue and purple color as well at the lower range, try it out, add it as another palette"): Hti Image's full rainbow, read back from its output (PRIOR_ART.md). It runs magenta, violet, blue, teal, green, yellow, orange and red to a softer red, 16 stops at even OKLab steps. It is the app's Rainbow HC.
...
Landscape by default (the View menu turns it: Orientation below), full-screen, minimal chrome. The controls live in the black margins beside the image (owner, 2026-09-26: "utilise them for quick action menus and items that I can toggle and activate"), so nothing covers the image and they needn't hide. At Full each margin is ~107 dp. Each bar's column hugs the screen's outer edge and is at most 150 dp wide, so the buttons stay under the thumbs and don't move when the view size changes.

- **Left bar (how the image looks):** the palette, the view and the box. The frame rate and lag show there too (debug), and while zoomed in, the zoom factor (a tap goes back to 1×).
  - *The palette tile opens a picker* (owner, 2026-09-26): White hot, Rainbow HC or Rainbow, each shown by its colours. Rainbow HC comes first of the rainbows (owner: "it look way better than the normal rainbow"). With Rainbow, its colours too: Deep, the default, or Soft. Stage 5's balance is on for all the rainbows.
  - *The View tile opens a menu* (owner, 2026-09-26): the view size and the orientation.
  - *Noise and Texture moved to Settings* (owner, 2026-09-26).
- **Menus** (owner, 2026-09-26: "should open to the right, also it should look better and modern and smooth"): a tile's menu opens beside it, toward the image: right of the left bar, left of the right bar, below the top row and above the bottom one. It grows out of that side as it fades in. Its choices are cards; the one chosen is tinted with an accent edge, and the highlight fades across as the choice changes (`Popups.kt`). The palette, the view and Capture's modes use it.
...
- **Settings** (owner, 2026-09-26: "simple presets that abstract settings into simple low and high effects"): each is Off / Low / High, kept across launches, and sets the stages underneath. Both default to Low (owner, 2026-09-26, after trying High), on the first Settings page rather than the bar.
  - *The recordings are on the first page too* (owner, 2026-09-26): a tile with their count opens the list. The logging switches are gone from the panel (the stats CSV, and the frames around each lockout): they're for test runs, driven over adb (`--ez csv`, `--ez lockoutDump`).
  - **Noise reduction:** stage 3c (the per-frame stripes) with stage 4b's BM3D (owner, 2026-09-26: the default method), at the strength that leaves the noise non-local means leaves at h 0.8 (Low) or 1.1 (High). Image quality comes first (owner, 2026-09-26; PIPELINE_LOG).
...
- **Orientation** (owner, 2026-09-26: "a menu for selecting view size as well as orientation (there should be auto orientation that changes with device orientation as well)"): the View menu picks Auto, Landscape (the default) or Portrait. Auto turns the screen all four ways with the tablet, whatever the system's rotation lock; the other two allow either way up. The choice is kept across launches.
  - *The image turns back by as much as the screen turned:* the camera is fixed to the tablet and turns with it (DEVICE.md). The image is upright at ROTATION_90 and turns `(1 − rotation) mod 4` quarters clockwise otherwise, in the vertex shader (`uRot`).
  - *Turned a quarter, the view is 3:4,* and a view-size preset gives its long side.
  - *Everything over the image follows the same turn* (`ViewGeometry.kt`): the markers, the box's outline, its hit-testing and edits (as the screen shows its sides), and pinch and pan.
  - *In portrait, the bars become rows* above and below the image, at most 96 dp tall, with the scale bar across (cold left, hot right). The Settings panel opens above the bottom row.
  - *Checked:* the harness renders the turn (`--rotate`; exact against plain rotations), and gpu_check passes turned, with zoom and the box (max 1 level). Portrait was checked live with the tablet on its stand (DEVICE.md); the owner still has to check it held in each orientation.
...
- **Box:** the Box button shows or hides a rectangle for measuring one area. It appears where it was last (initially centered, half the view's width and height). Drag handles on its edges and corners resize it; dragging inside it moves it. Touch targets stay large even when the box is small (hit-testing and minimum-size handling as in the Android croppers listed in PRIOR_ART.md). It snaps to whole camera pixels, is at least 4×4 of them, and is stored in camera coordinates, so view size and zoom don't disturb it. Outside the box, the image is dimmed (start at 50% brightness; tunable), keeping the box's color mapping — those pixels often clip at the palette ends.
  - *Built (2026-09-26):* the Box tile on the left bar.
  - *Editing:* a finger that lands on the box edits it. Well inside moves it; otherwise the nearest corner (within 28 dp), then the nearest edge. A second finger turns the gesture into a zoom (`BoxEditor.kt`).
  - *Rendering:* the renderer dims outside it (`uBox`, `uDim` 0.5). The harness renders the same with `--box`, and gpu_check passes with the box on (max 1 level).
...
- **Persistence:** the palette (and the rainbow's colours), the view size and orientation, the noise and texture settings and the box's last position and size are kept across launches; each launch starts in Auto range, at 1× zoom, with the box off.
