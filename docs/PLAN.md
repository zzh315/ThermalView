...
6. **Detail enhancement.** Guided-filter base/detail split on the raw signal, before tone mapping (owner decision, 2026-09-25; pass 2 item 8). Stage 5's curve compresses the base. The detail is added back, scaled by the curve's local slope, with a gain that tapers where local detail is near the noise floor (so noise isn't boosted) and a residual limiter so no halo forms. Sharpness has to come from here: no upscaler reaches the reference apps' edges (pass 2 item 8).
