...
- Start sequence (CLAUDE.md rule 1): start streaming at the verified format and frame interval → `0x8004` (InfiRay's demo waits 300 ms after starting the stream) → drop frames until they pass the sanity checks → `0x8020` → `0x8000` about 0.5 s later → drop frames through the shutter cycle. If streaming first gives trouble, switch to the approved fallback (`0x8004` and `0x8020` before streaming) and log why.
- The frame callback copies into a triple buffer and returns immediately. Process every frame; render the latest. A dedicated native thread runs libusb's event loop (PRIOR_ART.md, pass 1 items 1 and 6).
...
- Debug-only replay: plays a dump into the triple buffer at its recorded frame timing, looping, so everything downstream (pipeline, readouts, GPU, UI) runs exactly as it does with the camera. No camera needed; dumps from the Mac arrive with `adb push`.
...
1. **Shutter-cycle handling.** Detect frames captured during a shutter cycle (near-uniform frame or collapsing spatial variance, or the metadata signal M1 found, if any) and hold the last good output. Afterwards, blend back over ~0.3 s instead of jumping. Readouts hold their last values during a cycle too — a frozen or uniform frame gives meaningless numbers. Recalibration policy: unless M1 found the camera cycling by itself often enough, propose a periodic `0x8000` schedule from M1's drift measurements. Start from the vendor's 380 s, go shorter during warm-up (InfiCamPlus ramps from 7.5 s to 180 s), never below 10 s; a trigger on FPA-temperature change since the last shutter is an alternative. The owner approves the numbers; until then, only start-up and the Recalibrate button send `0x8000`.
...
- **View size:** the image always keeps its native 4:3 shape — no crop, no stretch. One button cycles Phone → Small tablet → Full, each defined as the image's physical diagonal using the panel density verified in M0. Starting values: Phone ≈ 4.5" (what a ~6.5" phone shows held sideways), Small tablet ≈ 7.5", Full = the largest 4:3 fit (2133×1600 px, ≈ 10.9" if the panel measures 12.4" at 2560×1600). The owner tunes them by eye. A smaller view adds no detail; it looks sharper because each camera pixel appears smaller, like viewing from farther away.
