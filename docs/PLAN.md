...
- **Left bar (how the image looks):** the palette, the view and the box. The frame rate and lag show there too (debug), and while zoomed in, the zoom factor (a tap goes back to 1×).
  - *The palette tile opens a picker* (owner, 2026-09-26): White hot, Rainbow or Rainbow HC, each shown by its colours. With Rainbow, its colours too: Deep, the default, or Soft. Stage 5's balance is on for all the rainbows.
